...
internal fun convert(root: Path, structs: Map<String, TypeStruct>) {
    ...
    buildExtensionDocumentation(extensions.blocks, extensionIDs)
...
private fun addFunction(node: StructuralNode) {
    val function = node.title
        .let { if (it.startsWith("PFN_xr")) it else it.substring(2) }
        .substringBefore('(')
        ...
        """<dl>
            ...
            it.items.asSequence()
                .map { entry ->
                    ...
                    """${entry.terms[0].text.let { if (it.isNotEmpty()) "<dt>${it.patch()}</dt>\n$t$t$t$indent" else "" }}<dd>${
                        if (entry.description.blocks.isEmpty())
