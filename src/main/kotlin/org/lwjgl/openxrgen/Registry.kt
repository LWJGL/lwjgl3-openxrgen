...
fun main(args: Array<String>) {
    ...
    val registry = openxrDocs.let {
        ...
        require(Files.isRegularFile(registryPath)) {
            "The path specified does not contain the OpenXR-Docs repository: $it"
        }
...
private fun getParams(
    ...
    params.asSequence().map { param ->
        ...
        val nullable = if ((indirection.isNotEmpty() || nativeType is TypeFuncpointer/* || (nativeType is TypeHandle && nativeType.type == "XR_DEFINE_HANDLE")*//* || nativeType is TypePlatform*/) && (
...
private fun getJavaImports(types: Map<String, Type>, fields: Sequence<Field>, enumRegistry: EnumRegistry) = fields
    .mapNotNull {
        if (it.array != null && it.array.startsWith("\"XR_"))
            "static org.lwjgl.openxr.${enumRegistry.enumImportMap[it.array.substring(1, it.array.length - 1)]!!}.*"
        else {
...
import org.lwjgl.generator.*${imports.asSequence()
        ...
        extension.requires.forEach { require ->
            val dependency = require.extension ?: require.feature.let {
                if (it == null) {
                    ...
                    val (major, minor) = XR_VERSION_REGEX.find(it)!!.destructured
                    "OpenXR$major$minor"
                }
            }
...
private fun PrintWriter.printCommands(
    ...
    commandRefs.forEach { commandRef ->
        ...
        if (commandRef == "xrGetInstanceProcAddr" || cmd.params.none { param ->
                param.indirection.isEmpty() && types.getValue(param.type).let { it is TypeHandle && it.type == "XR_DEFINE_HANDLE" }/* && param.optional != "true"*/
            }) {
            print("GlobalCommand..")
        }
